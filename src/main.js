...
(async () => {
    ...
    function buildWorkableQuery(type = "desktop") {
        return [
            {
                $match: {
                    $expr: {
                                ...
                                $lt: [
                                    {
                                        $sum: [
                                            {
                                                $convert: {
                                                    input: `$updated_at.last_page_speed_${type}_checked_at`,
                                                    to: "double",
                                                    onError: 0,
                                                    onNull: 0,
                                                },
                                            },
                                            86400 * 1000, // 1day to ms
                                        ],
                                    },
                                    new Date().getTime(),
                                ],
            ...
            {
                $sort: {
                    [`updated_at.last_page_speed_${type}_checked_at`]: 1,
                },
            },
            { $limit: 1 },
        ];
    ...
    async function assignPagesToLighthouseDesktopWorkable() {
        while (true) {
            console.log(
                "Getting page for desktop speed check " + new Date().toString()
            );

            if (_.size(lighthouseWorkableDesktop) !== 2) {
                let queryPipeline = buildWorkableQuery("desktop");

                let pagesForWork = await pagesCollection
                ...
                if (pagesForWork.length) {
                    ...
                    if (
                        !lighthouseWorkableDesktop.hasOwnProperty(
                            pageForWork.id
                        )
                    ) {
    ...
    async function assignPagesToLighthouseMobileWorkable() {
        while (true) {
            console.log(
                "Getting page for mobile speed check " + new Date().toString()
            );

            if (_.size(lighthouseWorkableMobile) !== 2) {
                let queryPipeline = buildWorkableQuery("mobile");
                ...
                if (pagesForWork.length) {
                    ...
                    if (
                        !lighthouseWorkableMobile.hasOwnProperty(pageForWork.id)
                    ) {
                        console.log("Assigning for work " + pageForWork.id);
    ...
    function generateLighthouseDesktopResult(id) {
        ...
        shell.exec(
            "lighthouse " +
                url +
                ' --emulated-form-factor=desktop --chrome-flags="--headless --no-sandbox" --output=json --output-path=./src/results/desktop/' +
                id +
                ".json",
            { async: true },
            async function (code, stdout, stderr) {
                await pagesCollection.updateOne(
                    ...
                    {
                        $set: {
                            "updated_at.last_page_speed_desktop_checked_at": new Date()
                                .getTime()
                                .toString(),
                        },
                    }
    ...
    function generateLighthouseMobileResult(id) {
        ...
        shell.exec(
            "lighthouse " +
                url +
                ' --emulated-form-factor=mobile --chrome-flags="--headless --no-sandbox" --output=json --output-path=./src/results/mobile/' +
                id +
                ".json",
            { async: true },
            async function (code, stdout, stderr) {
                await pagesCollection.updateOne(
                    ...
                    {
                        $set: {
                            "updated_at.last_page_speed_mobile_checked_at": new Date()
                                .getTime()
                                .toString(),
                        },
                    }
    ...
    function getDesktopWorkableEntry() {
        return _.findKey(lighthouseWorkableDesktop, ["working", false]);
    }

    function getMobileWorkableEntry() {
        return _.findKey(lighthouseWorkableMobile, ["working", false]);
    }
    ...
    async function desktopExecute() {
        while (true) {
            ...
            if (
                workableId &&
                lighthouseWorkableDesktop.hasOwnProperty(workableId)
            ) {
    ...
    async function mobileExecute() {
        while (true) {
            ...
            if (
                workableId &&
                lighthouseWorkableMobile.hasOwnProperty(workableId)
            ) {
    ...
    async function executeLighthouse() {
        return await Promise.all([desktopExecute(), mobileExecute()]);
    }

    async function parseDesktopResult() {
        while (true) {
            console.log(
                "Getting desktop results file list " + new Date().toString()
            );
            ...
            if (files.length > 0) {
                ...
                try {
                    let result = await fsp.readFile(
                        "./src/results/desktop/" + pickedFile,
                        "utf8"
                    );
                    ...
                    if (
                        result.hasOwnProperty("runtimeError") &&
                        Object.keys(result.runtimeError).length
                    )
                        return;
                    ...
                    if (
                        ...
                        result.timing.hasOwnProperty("total")
                    ) {
                    ...
                    if (result.hasOwnProperty("audits")) {
                        ...
                        Object.keys(result.audits).forEach((key) => {
                            ...
                            if (
                                result.audits[key].hasOwnProperty("description")
                            ) {
                    ...
                    if (result.hasOwnProperty("categories")) {
                        if (
                            ...
                            result.categories.performance.hasOwnProperty(
                                "auditRefs"
                            )
                        ) {
                        ...
                        if (
                            ...
                            result.categories.accessibility.hasOwnProperty(
                                "auditRefs"
                            )
                        ) {
                        ...
                        if (
                            result.categories.hasOwnProperty(
                                "best-practices"
                            ) &&
                            result.categories["best-practices"].hasOwnProperty(
                                "auditRefs"
                            )
                        ) {
                            delete result.categories["best-practices"][
                                "auditRefs"
                            ];
                        }
                    ...
                    console.log(
                        "Something wrong with desktop file " + pickedFile
                    );
                }
            }
    ...
    async function parseMobileResult() {
        while (true) {
            console.log(
                "Getting mobile results file list " + new Date().toString()
            );
            ...
            if (files.length > 0) {
                ...
                try {
                    ...
                    console.log(
                        "Something wrong with mobile file " + pickedFile
                    );
                }
            }
    ...
    async function parseLighthouseResults() {
        return await Promise.all([parseDesktopResult(), parseMobileResult()]);
    }

    async function afterLighthouseWork() {
        return await Promise.all([parseLighthouseResults()]);
    }
    ...
    await Promise.all([doLighthouseWork()]);
